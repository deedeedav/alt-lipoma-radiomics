...
# # Nested cross-validation structure with feature selection using Random Forest
...
def plotCM(cm, i, loop, ct_o, ct_i, k, l):
    ...
    plt.savefig(f'../../results/rf/combined/rf-ncv{i}-cm-{loop}-o{ct_o}i{ct_i}-C-{k}-Pen-{l}.png')
    plt.close()
...
def plot_learning_curve(
    ...
    axes[0].set_title("RFC Learning Curve")
    if ylim is not None:
...
def plotFI(model, features, i, ct):
    feat_importances = pd.Series(model.feature_importances_, index=features)
    feat_importances.nlargest(10).plot(kind='barh')
    plt.title("Feature importances")
    plt.savefig(f'../../results/rf/combined/ncv{i}-outer{ct}-feature-importance.png', bbox_inches='tight')
    plt.close()
...
f = open("../../results/rf/combined/rf-output.txt", "w")
...
# 
# # parameter search to find the best number of trees
# n_estimators = [1, 2, 4, 8, 16, 32, 64, 100, 200, 400, 800, 1000]
# train_results = []
# test_results = []
# 
...
#     train_pred = rf.predict(x_train0)
#     false_positive_rate, true_positive_rate, thresholds = roc_curve(y_train0, train_pred)
#     roc_auc = auc(false_positive_rate, true_positive_rate)
#     train_results.append(roc_auc)
#     y_pred = rf.predict(x_test0)
#     false_positive_rate, true_positive_rate, thresholds = roc_curve(y_test0, y_pred)
#     roc_auc = auc(false_positive_rate, true_positive_rate)
...
# We can see that we can stop at 200 trees to avoid further decrease in test performance.
...
# models to file
g = open('rf_final_models_combined.dat', 'wb')
...
for i in series:
    ...
    f.write(f'\nBest criterion: {model.criterion}')
    f.write(f'\nBest max features: {model.max_features}')
    f.write(f'\nBest max depth: {model.max_depth}')
    f.write(f'\nBest min samples split: {model.min_samples_split}')
    f.write(f'\nBest min samples leaf: {model.min_samples_leaf}')
...
ff = open('features_combined.txt', 'w')
for item in hp_features:
    ff.write(str(item) + "\n")
ff.close()

gg = open('hyperparams_combined.txt', 'w')
...
plt.savefig(f'../../results/rf/combined/ncv-rf-learning-curve.png')
plt.close()
...
# plt.plot(probs_pred, probs_true, marker='o', linewidth=1, label='RFC')
# 
...
# plt.legend()
# # plt.show()
# plt.savefig(f'../../results/rf/combined/rf-cal-curve.png')
# plt.close()
...
# plt.show()
plt.savefig(f'../../results/rf/combined/rf-roc.png')
plt.close()
# ------------------------------------------------------------------------------------------------------
...
df_excombined = pd.read_csv("../../data/clinical/clinical_data_external_combined.csv", sep=";", index_col=0)

df_excombined['ID_intern'].replace("LIP", "", regex=True, inplace=True)
df_excombined.drop(df_excombined.columns[1:6], axis=1, inplace=True)
df_excombined.drop(df_excombined.columns[2], axis=1, inplace=True)

df_ext = df_excombined.join(df_ext.set_index('ID_intern'), on='ID_intern')
...
h = open('rf_final_models_combined.dat', 'rb')
...
k = open('accuracies_models_combined.txt', 'w')
c=0
for model in models:
    c = c + 1
...
#plt.axes().set_aspect('equal', 'datalim')
# plt.show()
plt.savefig(f'../../results/rf/combined/rf-roc-external.png')
plt.close()
# ------------------------------------------------------------------------------------------------------------
...
plt.savefig(f'../../results/rf/combined/ncv-rf-learning-curve-external.png')
plt.close()
# ------------------------------------------------------------------------------------------------------------
...
plt.legend()
# plt.show()
plt.savefig(f'../../results/rf/combined/rf-cal-curve-external.png')
plt.close()
# -----------------------------------------------------------------------------------------------------------
